...
def add_marker_from_googleplaces():
...
def add_client_marker_not_in_google_places(mapa, dataframe):
    rows, columns = dataframe.shape
    for each_row in range(rows):
        client_id = dataframe.iloc[each_row, client_id_col]
        if not pd.isna(client_id):
            ...
            folium.Marker(
                        location=coords,
                        popup=str(dataframe.iloc[each_row, client_id_col]) + '<a href=' + link_wwp + '>Abrir Chat de Wwp</a>',
                        icon=icono_personalizado
                        ).add_to(mapa)
    return mapa
...
def add_markers(folium_map, dataframe, googleplaces_data, json_of_formatted_open_time):
    client_db = get_client_db()

    for place in googleplaces_data:
        if place['place_id'] in client_db:
            add_marker_with_combined_data(row, folium_map)
        else:
            add_marker_only_with_google_places_data(place, folium_map)

    for place_id in client_db:
        if not place_id and coords and frecuencia != '4-Muy Esporadico':
            add_marker_only_with_client_data(row, folium_map)

# config
map_output_name = 'ruta_sayago_map.html'
repo_path = r'C:\Work in Progress\Repos en GitHub\Mapa-Proyecto-Agricola'
wwp_col = letra_a_numero('H')
place_client_id_col = letra_a_numero('M')
coords_col = letra_a_numero('L')
client_id_col = letra_a_numero('I')
...
clientes_macetas_de_albahaca_path       = repo_path + r'\map-creator\data\data-mia\potenciales clientes para macetas de albahaca.tsv'
dados_de_baja_por_cualquier_motivo_path = repo_path + r'\map-creator\data\data-mia\dados de baja por cualquier motivo.tsv'
florerias_de_eventos_o_cementerios_path = repo_path + r'\map-creator\data\data-mia\florerias de eventos o cementerios.tsv'
...
add_markers(folium_map, dataframe, googleplaces_data, json_of_formatted_open_time)

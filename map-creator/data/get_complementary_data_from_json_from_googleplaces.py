    ...
    for entry in weekday_text:
        ...
        if 'Closed' not in hours:
            if 'Open 24 hours' in hours:
                formatted_entry.append(f"{day_es}: Abierto las 24 horas")
            else:
                formatted_entry.append(f"{day_es}: {hours}")
    ...
    for place_data in load_or_create_json_data(input_json_path):
        ...
        if place_id not in (place['place_id'] for place in output_json_data):
            ...
            additional_info = {
                ...
                'ruta': None,
                'de_baja_por_cualquier_motivo': False
            }
...
output_json_filename = 'complementary_data.json'
